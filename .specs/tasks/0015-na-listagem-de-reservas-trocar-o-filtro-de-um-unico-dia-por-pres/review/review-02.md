...
architecture: Re-review da rodada 1: não havia finding de arquitetura para revalidar. No diff atual o filtro de período e intervalo permanece nas camadas já documentadas — Application resolve a janela, Domain só altera o contrato do repositório, Infra HTTP valida e adapta Inertia, React escreve a query via service. Sem violação de Infra → Application → Domain nem de ADR-003, e sem blocker/high novo causado pelo repair. security: Re-review da rodada 1: não havia finding de segurança para revalidar. GET /reservations continua atrás de auth; o FormRequest restringe period e datas; o controller usa validated(); a query Eloquent é parametrizada; o React só interpola JSX. Nenhum vetor novo de authn/authz, injeção ou XSS foi evidenciado no repair. smells: Re-review da rodada 1: não havia smell blocker/high para revalidar. O código do filtro continua local e legível: resolveWindow tem um fluxo fechado, o FormRequest não esconde contrato com mixed, e a página React extrai presets/intervalo sem hooks condicionais nem any. Sem smell de correção ou manutenção que suba a blocker/high após o repair. tests: Re-review da rodada 1: não havia finding de testes para revalidar. A estratégia segue docs/test: unit cobre janela do use case, contrato do FormRequest e URL da página React; Feature cobre GET real com MySQL para presets, intervalo, paginação e um 422 na rota. E2E não se aplica e não foi inventado. Sem cenário duplicado entre níveis, teste no nível errado, ou regressão de proteção causada pelo repair.
...
- Index.jsx:87 visita /reservations só por visitIndex; presets e intervalo não filtram no cliente.
...
- Index.jsx:348 renderiza title/responsible/starts_at via JSX, sem dangerouslySetInnerHTML.
...
- Index.jsx:118 applyRangeField só visita com as duas datas e zera o par se uma for apagada depois de completo.
- Index.jsx:239 inputs type=date para Data inicial/Data final, sem time ou datetime-local.
...
- Index.test.jsx:126 clique em Hoje grava period e omite o intervalo; linha 194 range completo Y-m-d sem hora; linha 233 type=date; linha 243 Limpar filtros.
