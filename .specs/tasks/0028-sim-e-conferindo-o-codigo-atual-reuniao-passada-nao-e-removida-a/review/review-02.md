...
architecture: Re-review: o filtro de status continua Infra → Application → Domain. FormRequest valida o enum, o controller adapta HTTP/Inertia com default active, o caso de uso só encaminha e os predicados cancelled_at ficam em listPage. Ocupação permanece whereNull em hasActiveOverlap. Nenhum blocker/high novo após a rodada anterior. security: Re-review: GET /reservations segue atrás de auth; status é allowlist no FormRequest e entra no query builder por comparação, não por SQL interpolado. Ocupação continua ignorando canceladas. A UI esconde Editar/Cancelar, mas o PATCH de cancelamento não foi alterado neste diff. Nenhum blocker/high novo. smells: Re-review: SMELL-001 da rodada 1 não foi tratado — StatusBadge ainda usa fallback morto com os dois ramos Ativa. Nenhum blocker/high novo; não se abre lista nova de mediums fora desse ponto revalidado. tests: Re-review: o comportamento novo permanece no nível certo — validação e caso de uso em Unit, Eloquent/HTTP em Feature, UI em Vitest. Occupancy fica no Feature de cancel existente. E2E não foi exigido. Testes válidos não foram enfraquecidos. Nenhum blocker/high novo.
...
- ListReservations encaminha status a listPage sem HTTP/Eloquent em app/Modules/Reservation/Application/UseCases/ListReservations.php:37
