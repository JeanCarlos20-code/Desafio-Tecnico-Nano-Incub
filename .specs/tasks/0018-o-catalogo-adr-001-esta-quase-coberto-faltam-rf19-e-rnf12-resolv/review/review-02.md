...
architecture: Re-review após repair: a rodada 1 não tinha blocker/high. O login permanece em Infra HTTP (AuthenticatedSessionController + LoginRequest::authenticate), com a exceção já registrada em architecture.md e ADR-007. Seeders Laravel e a migration de catálogo não atravessam Application nem reintroduzem camadas. Direção Infra → Application → Domain e tree.md continuam. Nenhum novo desvio estrutural veio da correção. security: Re-review após repair: a rodada 1 não tinha blocker/high. POST /login segue com Auth::attempt só de e-mail e senha, remember-me falso, erro genérico em credentials (também no throttle) e regenerate de sessão. Logout invalida a sessão e o token CSRF. Painel continua atrás de auth; extras do Breeze não foram publicados. Schema ADR-002, HASH_DRIVER=argon e SoftDeletes permanecem. Nenhum novo risco de auth/sessão/segredo veio da correção. smells: Re-review após repair: a rodada 1 não tinha blocker/high. A troca continua sem use case paralelo: controller Breeze curto, FormRequest idiomático e seeders Eloquent determinísticos com firstOrCreate, sem factory nem User::create. Sem dead code de produção, $request->all() em persistência, catch vazio ou abstração especulativa. A correção não introduziu smell blocker/high. tests: Re-review após repair: a rodada 1 não tinha blocker/high. Validação de LoginRequest ficou em unit sem chamar authenticate(). O contrato HTTP/sessão em LoginHttpTest e DefaultAdministratorLoginHttpTest cobre Auth::attempt, throttle, soft-delete e regenerate. Seed, migrate-alone e 404 dos extras Breeze estão em Feature/MySQL 8. Suites Room/Reservation ajustaram contagens ao catálogo 3+3 sem apagar cenários. E2E Playwright não existe e duplicaria Feature+Vitest. Nenhum teste obrigatório foi enfraquecido a ponto de abrir blocker/high.
...
- app/Providers/AppServiceProvider.php:26 — binding de UserAuthenticator ausente; register só liga repositórios restantes
...
- docs/tree.md:20 — controller e FormRequest ficam em Modules/User/Infra/Http
...
- database/seeders/RoomSeeder.php:12 — três firstOrCreate com name/capacity/is_active literais, sem factory
- database/seeders/ReservationSeeder.php:17 — linhas determinísticas; firstOrFail nas salas nomeadas em vez de IDs mágicos
- app/Providers/AppServiceProvider.php:15 — imports do autenticador customizado permanecem ausentes
...
- tests/Feature/Database/DemoCatalogMigrationTest.php:15 — migrate sozinho já persiste as três salas ativas nomeadas
