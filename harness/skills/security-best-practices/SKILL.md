...
description: Perform language and framework specific security best-practice reviews and suggest improvements. Use when the user explicitly requests security best practices guidance, a security review or report, or secure-by-default coding help. Supports PHP/Laravel, Python, JavaScript/TypeScript, and Go. For Laravel + Inertia + React, load php-laravel-web-server-security.md and javascript-typescript-react-web-frontend-security.md. Do NOT use for general code review, debugging, threat modeling (use security-threat-model), or non-security tasks.
metadata:
...
If the project is PHP without Laravel, load `php-general-backend-security.md`. If it is Laravel (optionally Inertia), load `php-laravel-web-server-security.md` plus `php-general-backend-security.md`. Laravel + React/Inertia also requires the React frontend reference.
